    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
          <CalendarIcon className="w-6 h-6 text-emerald-600" /> Book Lunch
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
          <div>
            ...
            <input
              ...
              className="px-4 py-2.5 rounded-xl border border-slate-300 font-bold text-slate-900 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none"
            />
          </div>
        ...
        {loading ? (
          ...
          <div>
            {cutoff && !cutoff.isOpen ? (
              <div className="p-6 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-sm font-medium flex items-center gap-3">
              ...
              <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-slate-900">Lunch Booked for {selectedDate}</div>
                    <div className="text-xs text-slate-600">
                      Selection: <strong>{booking.foodOption.name}</strong> (₹{booking.priceAtBooking})
                    </div>
                  </div>
                </div>
                <button
                  ...
                  disabled={submitting}
                  className="px-4 py-2 rounded-xl bg-white border border-red-200 text-red-600 text-xs font-bold hover:bg-red-50"
                >
                  Cancel Booking
                </button>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  {data?.foodOptions?.map((opt: any) => (
                    <div
                      ...
                      className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                        foodOptionId === opt.id ? "border-emerald-500 bg-emerald-50/50" : "border-slate-200 bg-white"
                      }`}
                    >
                ...
                <button
                  onClick={handleBook}
                  disabled={submitting}
                  className="w-full py-3.5 rounded-xl gradient-emerald text-white font-bold text-sm shadow-md"
                >
